...
function createEvent(month, day, content, time, state) {
  ...
  divParentDetails.classList.add(
    "date",
    "row",
  ...
  // updating edit and delete btns
  btn_edit = document.querySelectorAll(".edit");
  btn_remove = document.querySelectorAll(".del");
  // active edit and delete btns after add event for new card
  edit();
  remove();
}
...
function remove() {
  btn_remove.forEach((element) => {
    element.addEventListener("click", () => {
      ...
      btn_remove = document.querySelectorAll(".del");
      console.log(
        "Length boxEvent " + boxEvent.length + "\n In Remove n = " + n
      );
    });
  });
}
